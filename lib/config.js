...
const DEFAULTS = {
  ...
  /**
   * How links are rendered in a tip: 'url' or 'none'.
   *
   * 'auto' and 'hyperlink' no longer mean anything here and are treated as
   * 'url'. Claude Code 2.1.252 sanitizes every tip — Bun.stripANSI, then every
   * \p{Cc}/\p{Cf} character deleted — so an OSC 8 hyperlink loses the URL along
   * with the styling and the tip arrives with nothing to follow. A visible URL
   * is the only form that survives; terminals linkify it themselves.
   *
   * 'none' still suppresses the URL, for anyone who wants the claim alone.
   */
  linkStyle: 'url',
  /**
   * Inert for tips, kept so an existing config.json is not a validation error.
   *
   * All three of these emit escape sequences, and the tip surface strips them
   * before display. Colour lives on the status line now — see statusLine.style
   * below, which is not sanitized.
   */
  linkColor: 'none',
  categoryColor: true,
  italicAction: true,
  ...
  statusLine: {
    enabled: true,
    /**
     * How the status line is tinted: 'lightBlue' | 'lavender' | 'periwinkle'
     * | 'grey' | 'blue' | 'green' | 'dim' | 'plain'.
     *
     * Was hardcoded to ANSI faint, which many terminals render as near-black.
     * Grey replaced it and was legible but sat at the same weight as a typical
     * first status line, so the two read as one block. Every option here avoids
     * the category palette, so the status line never shares a hue with a tip
     * category.
     */
    style: 'lightBlue',
  ...
  providers: {
    learn: {
      ...
      contextShare: 0.4,
      // How long a `lifespan: "timely"` card stays in rotation. It fades
      // linearly across this window and is retired at the end of it, so
      // harvested news ages out instead of accumulating.
      timelyTtlDays: 30,
    },
    wellness: {
      enabled: true,
