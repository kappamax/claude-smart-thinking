...
function pickItem(cache, tick, now = Date.now()) {
  if (!cache) return null;
  // Same reasoning as the status line: time-scoped items carry their own
  // expiry, and this adapter can show a cached item before a stale-triggered
  // refresh has finished — which is exactly when an expired one would surface.
  const status = Array.isArray(cache.status)
    ? cache.status.filter((item) => item && item.text && (!item.expiresAt || item.expiresAt > now))
    : [];
...
function createOpenCodePlugin({ root, deps = {} }) {
  ...
  return async function SmartThinkingPlugin({ client, directory }, options = {}) {
    ...
    const show = async () => {
      const display = formatItem(pickItem(readJson(cacheFile), tick++, now()));
      if (!display) return;
      try {
